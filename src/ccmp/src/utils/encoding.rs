...
pub fn encode_packed(tokens: &[Token]) -> Result<Vec<u8>, EncodingError> {
    let mut max = 0;
    for token in tokens {
        check(token)?;
...
fn check(token: &Token) -> Result<(), EncodingError> {
    match token {
        Token::FixedBytes(vec) if vec.len() > 32 => {
            Err(EncodingError::InvalidToken(token.to_string()))
        }

        Token::Tuple(_) => Err(EncodingError::InvalidToken(token.to_string())),
        Token::Array(vec) | Token::FixedArray(vec) => {
            for t in vec.iter() {
                if t.is_dynamic() || matches!(t, Token::Array(_)) {
                    return Err(EncodingError::InvalidToken(token.to_string()));
                }
                check(t)?;
            }
            Ok(())
